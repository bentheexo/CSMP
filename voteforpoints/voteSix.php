<?php
		...
		{
			if($unixrtime>=$highTimeForIPunix6)
			{
							$points = 1+$STOREpoints;
							mysql_query("UPDATE vote_point SET blockendedtime6 = '$expirationtime' WHERE account_id='$accountid'");
							...
							echo "<meta HTTP-EQUIV=\"REFRESH\" content=\"0; url=$vote6 \">";
			}
			else
			{
							echo "$ipblocked";
		...
		{
			if($unixrtime>=$lastRecordTimeBlockended6)
			{
								$points = 1+$STOREpoints;
								mysql_query("UPDATE vote_point SET blockendedtime6 = '$expirationtime' WHERE account_id='$accountid'");
								mysql_query("UPDATE vote_point SET point='$points'WHERE account_id = '$accountid'");
