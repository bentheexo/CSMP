<?php
		...
		{
			if($unixrtime>=$highTimeForIPunix10)
			{
							$points = 1+$STOREpoints;
							mysql_query("UPDATE vote_point SET blockendedtime10 = '$expirationtime' WHERE account_id='$accountid'");
							...
							echo "<meta HTTP-EQUIV=\"REFRESH\" content=\"0; url=$vote10 \">";
			}
			else
			{
							echo "$ipblocked";
		...
		{
			if($unixrtime>=$lastRecordTimeBlockended10)
			{
								$points = 1+$STOREpoints;
								mysql_query("UPDATE vote_point SET blockendedtime10 = '$expirationtime' WHERE account_id='$accountid'");
								mysql_query("UPDATE vote_point SET point='$points'WHERE account_id = '$accountid'");
