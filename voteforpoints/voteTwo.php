<?php
		...
		{
			if($unixrtime>=$highTimeForIPunix2)
			{
							$points = 1+$STOREpoints;
							mysql_query("UPDATE vote_point SET blockendedtime2 = '$expirationtime' WHERE account_id='$accountid'");
							...
							echo "<meta HTTP-EQUIV=\"REFRESH\" content=\"0; url=$vote2 \">";
			}
			else
			{
							echo "$ipblocked";
	...
	{
		if($unixrtime>=$lastRecordTimeBlockended2)
		{
							$points = 1+$STOREpoints;
							mysql_query("UPDATE vote_point SET blockendedtime2 = '$expirationtime' WHERE account_id='$accountid'");
							mysql_query("UPDATE vote_point SET point='$points'WHERE account_id = '$accountid'");
